...
export default async function ProfilePage({ params, searchParams }) {
  ...
  // Back-compat: accept `/profile/<id>` and redirect to `/profile/<username>`
  if (!viewedUser) {
    ...
    if (byId) {
      ...
      redirect(`/profile/${encodeURIComponent(byId.username)}${suffix}`);
    }
  }
  ...
  const canViewSettings = session.user.id === viewedUser.id || !!session.user.isAdmin;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.profileHeader}>
        ...
        <h1 className={styles.profileTitle}>{viewedUser.username}</h1>
      </div>

      <ProfileClient
        user={viewedUser}
        gauntlets={shapedGauntlets}
        initialGauntletId={gauntletIdParam}
        canViewSettings={canViewSettings}
        passwordTargetUserId={viewedUser.id}
      />
    </div>
  );
}
